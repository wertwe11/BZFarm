...
use BZpoultryfarm\Notifications\ActionApprovals;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Auth;
use BZpoultryfarm\Mail\SendPassword;
use BZpoultryfarm\User;
use BZpoultryfarm\Customers;
use BZpoultryfarm\CustomerArchives;
use BZpoultryfarm\CustomerChanges;
...
class CustomersController extends Controller
{
    ...
    public function ViewCustomers()
    {
        ...
        $newcust = Customers::whereBetween('created_at', [$lastWk->toDateTimeString(), $today->toDateTimeString()])->orderBy('created_at', 'desc')->paginate(10);
        $activity = CustomerChanges::paginate(10);
    ...
    public function addCustomer(request $request)
    {
        ...
        //initial customer password, random
        $random = str_random(10);

        if (Auth::user()->access == 'Farm Hand') //if access is farm hand
        {
            $add = new Approvals();
            ...
            $add->save(); //save details

            $add = Approvals::orderBy('id', 'desc')->first();

            //notify admins and managers for approval 
        ...
        else //access other than farmhand
        {
        	...
        	$customers->save(); //save customer details

            //data variable for mail
            $data = [
